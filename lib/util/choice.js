...
exports.shouldShowExtraChoice = exports.getIndexOfAnswer = exports.validateInput = exports.isAllowedValue = exports.getErrorMessage = exports.getItemControlValue = exports.renderOptions = exports.getCodesToClearForExclusiveSelection = exports.isExclusiveOption = exports.getExtension = exports.getDisplay = exports.getSystem = exports.getOptions = exports.hasOptions = exports.hasCanonicalValueSet = void 0;
...
function getExtension(options, value) {
    if (!options || options.length === 0) {
        return undefined;
    }
    const option = options.find(o => o.type === value);
    return option === null || option === void 0 ? void 0 : option.extension;
}
exports.getExtension = getExtension;
function isExclusiveOption(options, code) {
    var _a;
    if (!code) {
        return false;
    }
    return !!((_a = getExtension(options, code)) === null || _a === void 0 ? void 0 : _a.find(ext => ext.url === help_1.IExtentionType.optionExclusive && ext.valueBoolean));
}
exports.isExclusiveOption = isExclusiveOption;
function getCodesToClearForExclusiveSelection(options, selectedCodes, code) {
    if (!selectedCodes || selectedCodes.length === 0) {
        return [];
    }
    if (isExclusiveOption(options, code)) {
        return selectedCodes.filter((selectedCode) => !!selectedCode);
    }
    return selectedCodes.filter((selectedCode) => !!selectedCode && isExclusiveOption(options, selectedCode));
}
exports.getCodesToClearForExclusiveSelection = getCodesToClearForExclusiveSelection;
...
function createRadiogroupOptionFromValueCoding(coding, readOnly, extension) {
...
function createRadiogroupOptionFromValueReference(reference, readOnly, extension) {
...
function createRadiogroupOption(type, label, disabled, extension = []) {
    return {
        type: type,
        label: label,
        disabled,
