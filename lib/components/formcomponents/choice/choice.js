...
class Choice extends React.Component {
    constructor(props) {
        ...
        this.getPDFValue = (item, answer) => {
            ...
            return Array.isArray(value) ? value.map(el => (0, choice_1.getDisplay)((0, choice_1.getOptions)(this.props.resources, item, containedResources), el)).filter((choice) => Boolean(choice)).join(', ') : value;
        };
        this.getAnswerValueCoding = (code, systemArg, displayArg) => {
            var _a, _b, _c;
            const { item, containedResources, resources } = this.props;
            const option = (_a = item.answerOption) === null || _a === void 0 ? void 0 : _a.find(o => { var _a; return ((_a = o.valueCoding) === null || _a === void 0 ? void 0 : _a.code) === code; });
            if (option && option.valueCoding) {
                const coding = Object.assign({}, option.valueCoding);
                const imageExtensionsFromOption = ((_b = option.extension) === null || _b === void 0 ? void 0 : _b.filter(ext => ext.url === help_1.IExtentionType.choiceImage ||
                    ext.url === help_1.IExtentionType.mainImage ||
                    ext.url === help_1.IExtentionType.image)) || [];
                if (imageExtensionsFromOption.length > 0) {
                    const existingUrls = new Set((_c = coding.extension) === null || _c === void 0 ? void 0 : _c.map(ext => ext.url));
            ...
            const options = (0, choice_1.getOptions)(resources, item, containedResources);
            const display = displayArg ? displayArg : (0, choice_1.getDisplay)(options, code);
            const system = systemArg ? systemArg : (0, choice_1.getSystem)(item, code, containedResources);
        ...
        this.handleCheckboxChange = (code) => {
            ...
            if (dispatch && code) {
                ...
                if ((0, choice_1.getIndexOfAnswer)(code, answer) > -1) {
                    ...
                    const options = (0, choice_1.getOptions)(resources, item, containedResources);
                    const codesToClear = (0, choice_1.getCodesToClearForExclusiveSelection)(options, this.getValue(item, answer), code);
                    codesToClear.forEach(selectedCode => {
                        var _a;
                        (_a = dispatch((0, newValue_1.removeCodingValueAsync)(path, this.getAnswerValueCoding(selectedCode), item))) === null || _a === void 0 ? void 0 : _a.then(newState => onAnswerChange(newState, path, item, responseAnswer));
                    });
                    (_b = dispatch((0, newValue_1.newCodingValueAsync)(path, coding, item, true))) === null || _b === void 0 ? void 0 : _b.then(newState => onAnswerChange(newState, path, item, responseAnswer));
                }
                if (promptLoginMessage) {
        ...
        this.renderCheckbox = (options) => {
            var _a;
            const showChoiceImage = this.props.item.extension ? (_a = this.props.item.extension.find((extension) => extension.url === help_1.IExtentionType.choiceImage)) === null || _a === void 0 ? void 0 : _a.valueBoolean : false;
            return (React.createElement(checkbox_view_1.default, Object.assign({ showChoiceImage: showChoiceImage, options: options, id: this.props.id, handleChange: this.handleCheckboxChange, selected: this.getValue(this.props.item, this.props.answer), onRenderMarkdown: this.props.onRenderMarkdown }, this.props), this.props.children));
        };
